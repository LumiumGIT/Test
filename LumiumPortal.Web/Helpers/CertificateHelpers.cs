...
public static class CertificateHelpers
{
    ...
    public static string GetStatusText(CertificateStatus status) => status switch
    {
    ...
    public static string GetDaysText(int days) => days < 0
        ? $"Istekao pre {Math.Abs(days)} dana"
        : $"{days} dana";

    public static string GetDaysChipStyle(CertificateStatus status) => status switch
    {
